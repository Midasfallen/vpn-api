...
def create_tariff(t: schemas.TariffCreate, db: Session = Depends(get_db)):
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=400, detail="Tariff already exists or DB error")
    db.refresh(new)
    return new
...
def delete_tariff(tariff_id: int, db: Session = Depends(get_db)):
    ...
    if assigned:
        raise HTTPException(status_code=400, detail="Tariff is assigned to users and cannot be deleted")
    db.delete(tariff)
    db.commit()
