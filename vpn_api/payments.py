...
def create_payment(payload: schemas.PaymentCreate, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
    ...
    if payload.user_id and not getattr(current_user, 'is_admin', False) and current_user.id != payload.user_id:
        raise HTTPException(status_code=403, detail='Not allowed')
    payment = models.Payment(user_id=payload.user_id, amount=payload.amount, currency=payload.currency, provider=payload.provider)
    db.add(payment)
    db.commit()
    db.refresh(payment)
    return payment
...
def list_payments(user_id: Optional[int] = None, skip: int = 0, limit: int = 100, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
...
def update_payment(payment_id: int, payload: schemas.PaymentCreate, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
    payment = db.query(models.Payment).filter(models.Payment.id == payment_id).first()
    if not payment:
        raise HTTPException(status_code=404, detail='Payment not found')
    if not getattr(current_user, 'is_admin', False) and payment.user_id != current_user.id:
