...
curl -sS -X GET http://127.0.0.1:8000/auth/me -H "Authorization: Bearer $TOKEN" -o /tmp/me.json -w 'http_code:%{http_code}\n'
...
# create peer with user_id substituted by shell expansion
cat > /tmp/peer.json <<JSON
{"user_id": $USER_ID, "name":"test-wg-easy-final","allowed_ips":"10.99.99.10/32","dns":"1.1.1.1","description":"test created via wg-easy integration","wg_public_key":"somepubkey-$USER_ID","wg_ip":"10.99.99.10"}
JSON
